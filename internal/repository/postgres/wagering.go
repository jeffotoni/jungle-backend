...
func (s *Store) FindByBusiness(ctx context.Context, tx pgx.Tx, providerID, externalID string) (ports.WagerRecord, error) {
	return s.scanWager(tx.QueryRow(ctx, wagerSelect+` WHERE provider_id = $1 AND external_transaction_id = $2`, providerID, externalID))
}

func (s *Store) FindReference(ctx context.Context, tx pgx.Tx, providerID, externalID string) (ports.WagerRecord, error) {
...
func (s *Store) InsertWager(ctx context.Context, tx pgx.Tx, record ports.WagerRecord) (bool, error) {
	...
	_, err = tx.Exec(ctx, `
		...
		id, providerID, externalID, key, mustUUID(record.WalletID), record.PlayerID,
		nullIf(record.RoundID), nullIf(record.GameID), string(record.Kind), record.Amount,
		record.Currency, record.ReferenceExternalTransactionID, referenceID,
		string(record.Status), record.PayloadHash, record.ResultBalance, record.FailureCode)
	if err != nil {
		return false, err
	}
	var inserted bool
